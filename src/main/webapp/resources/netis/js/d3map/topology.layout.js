...
var D3Topology = {
        ...
        initialize: function () {
            ...
            this.createSvg('#' + this.vars.mapCanvas.parent().closest('div').attr('id') + ' > div[name=mapCanvas]');
    ...
    */
            ...
            if (e.altKey) {
                ...
                switch (e.code) {
                    case 'KeyE' : // 조회모드 <=> 관리모드
                        ...
                        menuValue = objTopo.isManageMode(objTopo) ? "modechg_search" : "modechg_manage"; break;
    ...
    searchLink: function () {
        ...
        Server.get("/d3map/topo/getTopoLinkList.do", {
            ...
            success: function (result) {
                ...
                /** 메모리 관련 문제로 임시 주석 처리 2023.03.22 */
                /* if (result.findIndex(findValue => findValue.lineTrafficEffect !== 'None') > -1 ) objTopo.ani_traffic(objTopo); */   // 회선 흐름 효과

                objTopo.tick(objTopo);
                objTopo.ani_bubble(objTopo);
                objTopo.playSound(objTopo);
                objTopo.vars.linkLabels = TopoLinkLabel.refresh(objTopo, objTopo.vars.svg, objTopo.vars.links);
    ...
    clickMenu: function (data) {
        ...
        if (objTopo.vars.mapMode == TopoConst.mapMode.MANAGE) {
            ...
            if (data == "close") {
                ...
                if($("#gSiteName").val() == "KSA") {

                    //KSA 예외처리. ( HTTP,HTTPS 제거)
    ...
    tick: function (objTopo) {
        ...
        if (objTopo.vars.links.length != 0) {
            ...
            objTopo.vars.svg.selectAll("path.link").attr("d", function (d) {
                ...
                //곡선 테스트
                if (d.lineType == 2) {
                    return ["M", sourceX, sourceY, "C", d.curveX1, d.curveY1, d.curveX2, d.curveY2, targetX, targetY].join(" ");
                }
    ...
    tickLinkLabels: function (objTopo) {
        ...
        linkLabelItem = objTopo.vars.linkLabels.filter( function (d, i) {
        ...
        if (!Array.isArray(linkLabelItem) &&  linkLabelItem.nodes().length > 0) TopoLinkLabel.refresh(objTopo, objTopo.vars.svg, objTopo.vars.links);
    },
    ...
    * 조회모드/관리모드 상태에 따른 설정
    ...
    checkAlignEdit: function (actionName) {
        ...
        var fIdx = topo_menu.checkActionList.findIndex(e => e.action === actionName);
        if (fIdx !== -1 ) {
            setTimeout(function () {
                if (objTopo.vars.isAlignEdit) {
