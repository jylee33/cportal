var Main = {
    ...
    initVariable: function () {
        ...
        Server.get('/engineer/popup/getLayoutMenuPage.do', {
            data: {inMenu: true},
            success: function (result) {
                $.each(result, function (idx, item) {
                    Main.layoutMenuList.push(item);
                    Main.layoutMenuList2.push(item);
                });
                    ...
                    try {
                        switch (value.codeId) {
                            ...
                            case "FIRST_PAGE_SETTING" :
                                ...
                                $('#FIRST_PAGE_LAYOUT').val(value.codeValue2);
                                if (value.codeValue4 != 'MAIN' && value.codeValue4 != 'LayOut'){
                ...
                if (_dashportList.length) {
                    ...
                    for (var i = 0, n = _dashportList.length; i < n; i++) {
                        ...
                        if (_codeData.codeValue1 === 'URL') {
                            ...
                            $('div[name=DASH_LAYOUT]:eq(' + _codeIdx + ')').val(_codeData.codeValue2);
    ...
    save: function () {
        ...
        if ($("#FIRST_PAGE_USE").val()){
            _list.push({
                ...
                codeValue1: HmResource.first_page_ver[1],
        ...
        $.each(_children, function (index, element) {
            ...
            if (_type === 'URL') {
                ...
                var _item = $(element).find('[name=DASH_LAYOUT]').jqxDropDownList('getSelectedItem')[0].originalItem;
        ...
        } else if ($('#FIRST_PAGE_TYPE').val() == 'Layout') {

            var _mainDashItem = $('#FIRST_PAGE_LAYOUT').jqxDropDownList('getSelectedItem').originalItem;
            _list.push({
                codeId: 'FIRST_PAGE_SETTING',
                codeValue1: $('#FIRST_PAGE_TYPE').val(),
                codeValue2: _mainDashItem.guid,
                codeValue3: _mainDashItem.grpType,
                codeValue4: 'LayOut',
                useFlag: 1
            });
        }
    ...
    addDashUrlTemplate: function () {
        ...
        var _template = '<div name="DASH_TYPE" style="float: left;margin-top:5px;"></div>' +
            ...
            '<div name="DASH_LAYOUT" style="float: left; display: none;margin-top:5px;"></div>' +
        ...
        $('div[name=DASH_TYPE]:eq(' + index + ')').jqxComboBox({
            ...
            .on('change', function (event) {
                ...
                if (data === 'URL') {
                    ...
                    $('div[name=DASH_LAYOUT]:eq(' + idx + ')').hide();
                    $('input[name=DASH_TEXT]:eq(' + idx + ')').prop('disabled', false);
                }
                else {
                    $('input[name=DASH_PORT]:eq(' + idx + ')').parent().hide();
                    $('div[name=DASH_LAYOUT]:eq(' + idx + ')').show();
                    $('input[name=DASH_TEXT]:eq(' + idx + ')').prop('disabled', true);
                }
            });

        $('div[name=DASH_LAYOUT]:eq(' + index + ')').jqxDropDownList({
            width: 200, height: 21,
            source: new $.jqx.dataAdapter({
                datatype: 'json',
                localdata: Main.layoutMenuList
            }),
    ...
    initMainPage: function () {
        ...
        $('div[name=FIRST_PAGE_TYPE]').jqxComboBox({
            ...
            .on('change', function (event) {
                ...
                var data = item.originalItem;
                if (data != 'Layout') {
                    $('div[name=FIRST_PAGE_LAYOUT]').hide();
                    ...
                    $('div[name=FIRST_PAGE_LAYOUT]').show();
                }

                $('#MAIN_PAGE_NAME').val("MAIN");
                $("#FIRST_PAGE_USE").val(false);

                //None, Layout이 아닐 경우, "시작페이지 설정"항목 on/off
                if (data != 'None' && data != 'Layout') {
        ...
        $('div[name=FIRST_PAGE_LAYOUT]').jqxDropDownList({
            width: 200, height: 21,
            source: new $.jqx.dataAdapter({
                datatype: 'json',
