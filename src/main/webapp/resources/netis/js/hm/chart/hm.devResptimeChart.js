/**
 * 장비 ResponseTime(응답시간) 차트
 * @param chartId
 * @constructor
 */
var DevResptimeChart = function(chartId) {
    this.chartId = chartId;
    this.chart = null;
};

DevResptimeChart.prototype = function() {
    ...
    var initialize = function() {
        this.chart =
            HmHighchart.createStockChart(this.chartId, {
                ...
                series: [
                    {name: '응답시간 평균', type: 'area'},
                    {name: '응답시간 최대', type: 'line'},
                    {name: '응답시간 최소', type: 'line', visible: false}
                    //{name: '응답시간 최대', type: 'line', lineWidth: 0, marker: {enabled: true, radius: 4, symbol: 'diamond'}, visible: false}
                ]
    ...
    var searchData = function(params) {
        ...
        if(esUse != 'Y'){
            perfData.searchIpPerf(_this, params, searchDataResult);
            // search(_this, params);
        }else{
