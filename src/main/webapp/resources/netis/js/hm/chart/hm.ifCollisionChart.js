 ...
 * @constructor
 */
var IfCollisionChart = function(chartId) {
    this.chartId = chartId;
    this.chart = null;
};

IfCollisionChart.prototype = function() {
    ...
    var searchDataResult = function(params, result) {
        ...
        if(params.tableCnt == 1) {
            ...
            chartDataArr = HmHighchart.convertJsonArrToChartDataArr('DT_YMDHMS', ['AVG_COLLISION'], result);
        }
        else {
            if(this.chart.series) {
                this.chart.series[1].show();
            }
            chartDataArr = HmHighchart.convertJsonArrToChartDataArr('DT_YMDHMS', ['AVG_COLLISION', 'MAX_COLLISION', 'MIN_COLLISION'], result);
        }
