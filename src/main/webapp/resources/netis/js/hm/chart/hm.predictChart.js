 ...
 * @constructor
 */
var PredictChart = function(chartId, chartType, options, url) {
    this.chartId = chartId;
    this.chartType = chartType;
    this.series = options.series || [];
    this.unit = options.hasOwnProperty('chartConfig')? options.chartConfig.unit : '';
    this.rangeInfo = options.hasOwnProperty('rangeInfo')? options.rangeInfo : null;
    this.url = url;
...
PredictChart.prototype = function() {
    /**
     * 차트 생성
     */
    ...
    var searchDataResult = function(params, result) {
        ...
        switch(this.chartType) {
            ...
            case HmHighchart.TYPE_COLUMN:
                ...
                var xFieldArr = [], yFieldArr = [], listNmArr = [];
                $.each(_series, function(si, sv) {
                    xFieldArr.push(sv.xField);
                    yFieldArr.push(sv.yField);
                    if(sv.hasOwnProperty('listNm')){
                        listNmArr.push(sv.listNm);
                    }
                    chartData[si] = [];
                });
                ...
                if(listNmArr.length){
                    $.each(result, function(i, v) {
                        var sidx = listNmArr.indexOf(i);

                        if(rangeInfo.type == 'DEV'){
                            $.each(v, function(idx, value) {
                                var _xField = xFieldArr[sidx], _yField = yFieldArr[sidx];
                                chartData[sidx].push([value[_xField], value[_yField]]);
                                var minus = value[_yField] - (value[_yField] * Math.abs(rangeInfo.errRate));
                                if(rangeInfo != null && rangeInfo.rangeFlag == true && rangeInfo.baseField == _yField && value[_xField] <= rangeInfo.currDate){
                                    chartData[_series.length-1].push([value[_xField], minus < 0 ? 0 : minus, value[_yField] + (value[_yField] * rangeInfo.errRate) ]);
                                }
                            ...
                            $.each(v, function(idx, value) {
                                ...
                                if(rangeInfo != null && rangeInfo.rangeFlag == true && rangeInfo.baseField == _yField && value[_xField] <= rangeInfo.currDate){
                                    var per = rangeInfo.linewidth * rangeInfo.errRate;
                                    var minus = value[_yField] - per;
                                    chartData[_series.length-1].push([value[_xField], minus < 0 ? 0:minus , value[_yField] + per]);
                                }
