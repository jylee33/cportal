...
var Main = {
	...
	/** init design */
	initDesign : function() {
		$('#editor').jqxEditor({ height : "337px", editable : false, disabled : true, tools : '' });
		$.ajax({ type : "post", url : $('#ctxPath').val() + '/main/oms/noticeBoard/getBoardContents.do', data : "boardNo=" + $('#boardNo').val(), dataType : "json", success : function(jsonData) {
				var contents = jsonData.resultData.contents;

				$('#userName').val(contents.userName + "(" + contents.userId + ")");

				var userNameSize = parseFloat($('#userName').val().length ) * 9  ;
				$('#userName').width( userNameSize + 'px' );

				$('#boardHits').val(contents.boardHits);
				$('#boardTitle').val(contents.boardTitle);
				$('#fullTimeFormat').val(contents.fullTimeFormat);
	...
	deleteBoard : function() {
		...
		if (confirm("삭제 하시겠습니까?") != true)
			return;
		$.ajax({ type : "post", url : $('#ctxPath').val() + '/main/oms/noticeBoard/delBoard.do', data : "boardNo=" + $('#boardNo').val(), dataType : "json", success : function(jsonData) {
			} });
