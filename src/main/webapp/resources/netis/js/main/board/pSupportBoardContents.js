...
var Main = {
		...
		initDesign: function() {
			...
			$.ajax({
				...
				success : function(jsonData) {
					...
					// 18.05.23] 이미 처리완료인 게시글에서 버튼 숨김
					var checkFlag = resultData.checkFlag;
					if(checkFlag==1){
		...
		upgradeBoard: function() {
			window.location.href=$('#ctxPath').val() +"/main/board/pSupportBoardEdit.do?boardNo="+$('#boardNo').val();
		},
		...
		deleteBoard: function() {
			...
			$.ajax({
				type : "post",
				url :$('#ctxPath').val() + '/main/oms/supportBoard/delBoard.do',
				data : "boardNo="+$('#boardNo').val(),
				dataType : "json",
				success : function(jsonData) {
		...
		commentBoard: function() {
			window.location.href=$('#ctxPath').val() +"/main/board/pSupportBoardComment.do?boardNo="+$('#boardNo').val();
		},
