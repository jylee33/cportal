...
observe : function() {
    ...
    $('#fileUpload').on('uploadEnd', function(event) {
        if (--uploadCnt == 0) {
            location.href = $('#ctxPath').val() + '/main/board/pSupportBoardContents.do?boardNo=' + boardNo;
...
/** init design */
initDesign : function() {
	$('#editor').jqxEditor({ height : "300px" });
    $('#fileUpload').jqxFileUpload({ width : '100%', height:'100px', fileInputName : 'fileinput', accept:'.xlsx,.xls,image/*,.doc, .docx,.ppt, .pptx,.txt,.pdf' });
    $("#jqxLoader").jqxLoader({text: "", isModal: true, width: 60, height: 36, imagePosition: 'top'});
	$.ajax({ type : "post", url : $('#ctxPath').val() + '/main/oms/supportBoard/getBoardContents.do', data : "boardNo=" + $('#boardNo').val(), dataType : "json", success : function(jsonData) {
    ...
    Server.post('/main/oms/noticeBoard/editBoard.do', {
        ...
        success: function (result) {
            ...
            if ($('.jqx-file-upload-file-row').length == 0) {
                location.href = $('#ctxPath').val() + '/main/board/pSupportBoardContents.do?boardNo=' + result.boardNo;
                window.opener.Main.searchSBoard();
                self.close();
            } else {
                try {
