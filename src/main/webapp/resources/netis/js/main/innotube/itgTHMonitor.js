...
var Main = {
    ...
    eventControl: function (event) {
        ...
        switch (curTarget.id) {
            ...
            case "btnExcel":
                ...
                break;
            case 'btnCList_te':
                this.showChartData('te');
                break;
            case 'btnCList_hu':
                this.showChartData('hu');
                break;
            case 'btnCSave_te':
                this.saveChart($teChart, 'temperature.png');
                break;
            case 'btnCSave_hu':
                this.saveChart($huChart, 'humidity.png');
                break;
        }
    },
    ...
    getCommParams: function () {
        ...
        $.extend(params, $.extend(params, HmBoxCondition.getPeriodParams()));
        return params;
    },
    ...
    search: function () {
        ...
        HmGrid.updateBoundData($grid, ctxPath + '/main/innotube/itgTHMonitor/getItgTHMonitorList.do');
    },
    ...
    drawChart: function (mngNo, sensorId) {
        ...
        Server.get('/main/innotube/itgTHMonitor/getItgTHMonitorChartList.do', {
            data: params,
    ...
    exportExcel: function () {
        HmUtil.exportGrid($grid, '온습도감시', false);
        // var params = Main.getCommParams();
        // HmUtil.exportExcel(ctxPath + '/main/innotube/itgTHMonitor/export.do', params);
    },
    ...
    showChartData: function (chartType) {
        ...
        if (chartType == 'te') {
            cols = [
                ...
                {text: '온도(℃)', datafield: 'temp', cellsalign: 'right', width: 150}
            ];
            chartData = $teChart.jqxChart('source');
        }
        else if (chartType == 'hu') {
            cols = [
                {text: '일시', datafield: 'ymdhms'},
                {text: '습도(%)', datafield: 'humidity', cellsalign: 'right', width: 150}
            ];
            chartData = $huChart.jqxChart('source');
        }
        else return;

        $.post(ctxPath + '/main/popup/comm/pChartDataList.do',
            function (result) {
                HmWindow.open($('#pwindow'), '차트 데이터 리스트', result, 600, 600, 'p2window_init', {
                    cols: cols,
                    chartData: chartData
                });
            }
        ...
        switch (val) {
            case 'devCpst':
                try {
                    ...
                    HmUtil.createPopup('/main/popup/innotube/pTHDevDetail.do', $('#hForm'), 'pDevDetail_' + params.mngNo, 1300, 700, params);

                    // $.post(ctxPath + '/main/popup/innotube/pTHDevDetail.do',
                    //     params,
                    //     function (result) {
                    //
