...
var Main = {
    ...
    initDesign : function() {
        ...
        HmTreeGrid.create($grpTree, HmTree.T_AP_GRP_DEFAULT, Main.selectTree);
    ...
    searchAsset: function() {
        ...
        Server.get('/main/nms/assetInfo/getApAssetItemList.do', {
            success : function(data) {
                ...
                $apAssetGrid.jqxGrid('beginupdate', true);
                $apAssetGrid.jqxGrid({ columns : cols });
                $apAssetGrid.jqxGrid('endupdate');

                // 데이터 갱신
                HmGrid.updateBoundData($apAssetGrid, ctxPath + "/main/nms/assetInfo/getApAssetInfoList.do");
            }
        });
    },
    ...
    delAsset : function() {
        ...
        var rowIdx = HmGrid.getRowIdx($apAssetGrid, '데이터를 선택해주세요.');

        if (rowIdx === false) return;

        if (!confirm('선택된 데이터를 삭제하시겠습니까?')) return;

        var rowdata = $apAssetGrid.jqxGrid('getrowdata', rowIdx);

        // 추가된 컬럼이면 로컬에서 삭제
        if (addIds.indexOf(rowdata.uid) !== -1 && rowdata.MNG_NO === undefined) {
            $apAssetGrid.jqxGrid('deleterow', rowdata.uid);
            delete addIds[addIds.indexOf(rowdata.uid)];
            return;
        }

        // DB데이터 삭제
        Server.post('/main/nms/assetInfo/delApAssetInfo.do', {
            data : rowdata,
...
var fixedCols = [
    ...
    { text : 'IP', datafield : 'AP_IP', width : 120,
        validation: function(cell, value) {
            ...
            if(!$.validateIp(value)) {
                return { result: false, message: 'IP를 확인해주세요.' };
            }
            return true;
        }
    },
