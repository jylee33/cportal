...
var pMibMain = {
    ...
    initVariable: function() {
        ...
        $mib_MibGrid = $('#mib_MibGrid'); // mib module
    },

    /** add event */
    observe: function() {
        $('button').bind('click', function(event) { pMibMain.eventControl(event); });
    },
    ...
    eventControl: function(event) {
        ...
        switch(curTarget.id) {
            ...
            case 'btnSave_dtl': this.saveDtlInfo(); break;
            case 'pbtnClose': self.close(); break;
            case "btnAdd_pMidModule": pMibMain.addMibModule(); break;
        }
    },
    ...
    initDesign: function() {
        ...
        //     pMibMain.selectDevCtxmenu(event);
        // });

        // 메인텝
        $mibDtlTab.jqxTabs({ width: '99.8%', height: '99%', theme: 'ui-hamon-v1-tab-top',
...
//				pMibMain.searchDtlInfo();
            });

        $('#p_mibSplitter').jqxSplitter({ width: '100%', height: '100%', orientation: 'horizontal', theme: jqxTheme, panels: [{ size: 250, collapsible: false  }, { size: '100%' }] });

        // mib module 그리드
        HmGrid.create($mib_MibGrid, {
            ...
            pageable: false,
            columns : [
                { text : '항목', datafield : 'moduleName', width : '100%' },
                { text : '경로', datafield : 'modulePath', hidden : true }
            ]
        }, CtxMenu.NONE);
    ...
    addMibModule: function () {
        Server.post('/main/popup/mibBrowser/loadMibModule.do', {
            data: { test : 'tsets'},
            success: function() {
                // $.each(_rowids, function(idx, rowid) {
                //     $grid.jqxGrid('setcellvaluebyid', rowid, 'status', '진행중');
