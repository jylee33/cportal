...
var PMain = {
		...
		eventControl: function(event) {
			...
			switch(curTarget.id) {
                ...
                case "btnExcel_evtHist": PMain.exportExcel(curTarget.id); break;
                case "btnSearch_evtActionHist": PMain.searchHistory(true); break;
                case "btnExcel_evtActionHist": PMain.exportExcel(curTarget.id); break;
			}
		},
		...
		initDesign: function() {
            ...
            // 메인텝
			$dtlTab.jqxTabs({ width: '99.8%', height: '99%', theme: 'ui-hamon-v1-tab-top', selectedItem: 1,
				initTabContent: function(tab) {
					...
					case 1: // 성능
						...
						break;
					case 2: // 이벤트
                        $eventTabs.jqxTabs({width: '100%', height: '100%',
                            initTabContent: function(tab) {
                                switch(tab) {
                                    case 0://현황
                    ...
                    if(event.args.rightclick) {
                        ...
                        if(idxes.length > 1) {
                            ...
                            if(_list.length > 1) targetMenu = $('#ctxmenu_evt');
                                ...
                                switch(_list[0].srcType) {
                                    case 'DEV': targetMenu = $('#ctxmenu_dev_action'); break;
                                    case 'IF': targetMenu = $('#ctxmenu_if_action'); break;
                                    case 'AP': targetMenu = $('#ctxmenu_ap_action'); break;
                                    case 'ITMON': targetMenu = $('#ctxmenu_itmon_action'); break;
                                    case 'SYSLOG': targetMenu = $('#ctxmenu_syslog_action'); break;
