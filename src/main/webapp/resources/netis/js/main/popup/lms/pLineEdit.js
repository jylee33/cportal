...
var PMain = {
	...
	eventControl: function(event) {
		...
		switch(curTarget.id) {
			case 'pbtnSave': PMain.btnAdd(); break;
	...
	initDesign: function() {
		...
		HmGrid.create($('#addGridList'), {
			source: new $.jqx.dataAdapter(
				{
					datatype: 'json',
					url:ctxPath + '/main/lms/lineMgmt/getLinkList.do'
				},
				{
					formatData: function(data) {
						$.extend(data, { connSeqNo: $('#pSeqNo').val()});
						return data;
					},
					loadComplete: function(records) {
						editAddGridIds = [];
			...
			columns:
				[
					...
					{ text : '상위 회선', columngroup:'up',datafield : 'upIfIdxUp', minwidth : 60, editable: false, hidden: true},
					{ text : '하위 회선', columngroup:'up',datafield : 'upIfIdxDown', minwidth : 60, editable: false, hidden: true},
					{ text : '상위 회선', columngroup:'up',datafield : 'upIfIdxUpName', minwidth : 60, editable: false},
					{ text : '하위 회선', columngroup:'up',datafield : 'upIfIdxDownName', minwidth : 60, editable: false},
					...
					{ text : '상위회선', columngroup:'down',datafield : 'downIfIdxUp', minwidth : 60, editable: false,hidden: true},
					{ text : '하위 회선', columngroup:'down',datafield : 'downIfIdxDown', minwidth : 60, editable: false,hidden: true},
					{ text : '상위회선', columngroup:'down',datafield : 'downIfIdxUpName', minwidth : 60, editable: false},
					{ text : '하위 회선', columngroup:'down',datafield : 'downIfIdxDownName', minwidth : 60, editable: false}
				],columngroups: [
...
//				_grpNo = p_grpTree[0].grpNo;
		...
		if(upDevIdx === false || downDevIdx === false) {
			...
			_upDevName = upDevItem.disDevName;
			_upGrpNo = upDevItem.grpNo;
			_upGrpName = upDevItem.grpName;

			_downMngNo = downDevItem.mngNo;
			_downDevName = downDevItem.disDevName;
			_downGrpNo = downDevItem.grpNo;
			_downGrpName = downDevItem.grpName;
		}
		...
		var data = {
			...
			upIfIdxUp: _upIfIdxu,
			upIfIdxDown: _upIfIdxd,
			upIfIdxUpName: _disUpIfIdxu,
			upIfIdxDownName: _disUpIfIdxd,
			downMngNo: _downMngNo,
			downDevName: _downDevName,
			downGrpNo: _downGrpNo,
			downGrpName: _downGrpName,
			downIfIdxUp: _downIfIdxu,
			downIfIdxDown: _downIfIdxd,
			downIfIdxUpName: _disDownIfIdxu,
			downIfIdxDownName: _disDownIfIdxd
		}
...
// 		},
	...
	btnDelGrid: function(){
		...
		if(rowIdx-1 < 0){
			...
			if(!confirm('최근 추가한 연결정보를 삭제하시겠습니까?')) return;
	...
	btnAdd: function(){
		...
		if(rows != 0) {
			...
			$.each(rows, function (idx, value) {
				...
				if (rows.length > idx) {
					...
					_list.push({ 'upMngNo': data.upMngNo, 'upIfIdxUp': data.upIfIdxUp, 'upIfIdxDown': data.upIfIdxDown, 'downMngNo': data.downMngNo, 'downIfIdxUp': data.downIfIdxUp, 'downIfIdxDown': data.downIfIdxDown, lineCnt: cnt++, sLocKind: sLocKind, eLocKind: eLocKind });
				}
			});
			obj.list = _list;
		}
		obj.connSeqNo = $('#pSeqNo').val();
		...
		Server.post('/main/lms/lineMgmt/editLine.do', {
			data : obj,
