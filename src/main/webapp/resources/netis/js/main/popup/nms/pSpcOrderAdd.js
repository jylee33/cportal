...
var PMain = {
    ...
    save: function () {
		...
		Server.post('/main/nms/spcOrder/addSpcOrder.do', {
			data: params,
    ...
    createOrderForm: function () {
        ...
        $.ajax({
			type:'get',
			url: '/main/nms/spcOrder/getSpcOrderForm.do',
            contentType: 'html',
            success: function (form) {
                DesAreaDocnt.body.innerHTML = form;
				//form 페이지 로드 이후 배경 제어용 div 세팅
	...
	editEtc: function (tag, readTag) {
		...
		switch (tag) {
			...
			case styleMode.ADD_HR1:
				...
				if(hrHeight) { hrColor = prompt("◎ 수평선의 색상을 지정하거나 [확인] 버튼을 누르십시오.", hrColor);}
				if(hrColor) { o.innerHTML += "<hr size=" + hrHeight + "  width=" + hrWidth + "  color=" + hrColor + "><br/>";}
				break;
