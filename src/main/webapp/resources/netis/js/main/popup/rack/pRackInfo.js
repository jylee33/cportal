...
var Rack = {
    ...
    initCondition: function (type) {
        var list = type === 'SINGLE' ?
            [{label:'앞면', value:Rack.SECTION.FRONT}] :
            [{label:'앞면', value:Rack.SECTION.FRONT}, {label:'뒷면', value:Rack.SECTION.BACK}]
        ;
        HmBoxCondition.createRadio($('#sSrchType'), JSON.parse(JSON.stringify(list)));
    },

    initDesign: function () {
        //rack no
        this.rackNo = $('#pRackNo').val();
        this.mngNo = $('#pMngNo').val();
        Server.post('/main/env/rackConf/getRackInfo.do', {
            ...
            success: function(result) {
                ...
                $('#p_rackU').val(result.rackU);
                $('#p_width').val('{0} / {1}'.substitute(result.width, result.height));
                $('#p_ymd').val(result.ymd);
                $('#p_model').val(result.model);
                $('#p_vendor').val(result.vendor);

                Rack.initSlotNo();
                Rack.initDiagram();
                Rack.initData();
                Rack.initCondition(result.rackType);

                Rack.initRackInfoSlot();
                Rack.initVirtualSvr();
                Rack.initEvent();
            }
        });
    ...
    observe: function() {
        ...
        $('#sSrchType').change(Rack.changeType);
    },
    ...
    setRackData: function (_type) {
        ...
        //우측 정보
        _type === Rack.SECTION.FRONT ?
            Rack.rackInfoDiagram.model.nodeDataArray = backInfo :
            Rack.rackInfoDiagram.model.nodeDataArray = frontInfo
    },
    ...
    initData: function() {
        Server.get('/main/popup/rackConf/getRackSlotInfo.do', {
            ...
            success: function(result) {
                ...
                // $('#p_rackSection').text(result.rackSection)
                ...
                $.each(infoResult, function (idx, item) {
                    item.rackSection === Rack.SECTION.FRONT ?
                        frontInfo.push(item) : backInfo.push(item);
                });
                // Rack.rackInfoDiagram.model.nodeDataArray = infoResult.filter(function(data){return data.category != 'bld'});
                Rack.rackInfoDiagram.model.nodeDataArray = frontInfo.filter(function(data){return data.category != 'bld'});

                var _nodeDataArray = Rack.myDiagram.model.nodeDataArray;
                $.each(_nodeDataArray, function(idx, item) {
                    if(Rack.mngNo == item.mngNo){
                        Rack.myDiagram.select(Rack.myDiagram.findNodeForKey(item.key));
                        Rack.rackInfoDiagram.select(Rack.rackInfoDiagram.findNodeForKey(item.key));
    ...
    initSlotNo: function(slotU) {
        ...
        $('#rackInfoDiv').parent('div:first').height(this.rackSlotU * 20 + 10); //div 높이조절
        var ul = $('#slotNumberUL');
        for(var i = this.rackSlotU; i >= 1; i--) {
            ul.append($('<div></div>', { text: i }));
        }
    },
    initRackInfoSlot: function() {
        ...
        this.rackInfoDiagram =
            $(go.Diagram, "rackInfoDiv",
                {
                    grid: $(go.Panel, "Grid", //or go.Panel.Grid
                        ...
                        $(go.Shape, "LineH", { stroke: "#777e84" }),
                        $(go.Shape, "LineV", { stroke: "#777e84" })
                    ),
        ...
        var svrNodeTemplate =
            $(go.Node, "Auto",
                ...
                $(go.Panel, 'Horizontal',
                    ...
                    $(go.TextBlock,
                        { row: 0, column: 0, width: 40, height: 20, verticalAlignment: go.Spot.Center, alignment: go.Spot.Center, textAlign: "center", font: 'bold 11px sans-serif', stroke: '#fff', background: "#529cc6" },
                        new go.Binding('text', 'orgSlotU', function(v) {   return v + 'U'  })),


                    $(go.Shape, "Rectangle", { row: 0, column: 0, stroke: "#e5f1fa",fill: "#e5f1fa", width: 2, height: 20 }),

                    $(go.TextBlock,
                        { row: 0, column: 0, width: 48, height: 20, verticalAlignment: go.Spot.Center, alignment: go.Spot.Left, font: 'bold 11px sans-serif', stroke: '#000', background: "#e5f1fa" },
                        new go.Binding('text', 'descSlotNo', function(v) { return Rack.infoRenderer(v + '슬롯'); })
                    ),

                    $(go.Shape, "Rectangle", { row: 0, column: 0, stroke: "#e5f1fa",fill: "#e5f1fa", width: 10, height: 20 }),

                    /* 임시.. 정보없음*/
                    $(go.Picture,{ row: 0, column: 0,  width: 60, height: 20,  background: "#e5f1fa", imageStretch: go.GraphObject.Uniform },
                        new go.Binding("visible", "slotName", function(s) { if (s) return false; else return true; }),
                        new go.Binding('source', '', function(data) { return '/img/rack/noInfo.png'})),

                    $(go.Shape, "Rectangle", { row: 0, column: 0, stroke: "#e5f1fa",fill: "#e5f1fa", width: 40, height: 20 },
                        new go.Binding("visible", "slotName", function(s) { if (s) return false; else return true; })),

                    $(go.TextBlock,
                        { row: 0, column: 0, width: 100, height: 20, verticalAlignment: go.Spot.Center, alignment: go.Spot.Left, font: 'bold 11px sans-serif', stroke: '#000', background: "#e5f1fa" },
                        new go.Binding('text', 'slotName', function(v) { return Rack.infoRenderer(v, true, 14); }),
                        new go.Binding("visible", "slotName", function(s) { if (s) return true; else return false; })
                    ),

                    $(go.Shape, "Rectangle", { row: 0, column: 0, stroke: "#e5f1fa",fill: "#e5f1fa", width: 20, height: 20 }),

                    /* 임시.. 정보없음*/
                    $(go.Picture,{ row: 0, column: 0,  width: 60, height: 20,  background: "#e5f1fa", imageStretch: go.GraphObject.Uniform },
                        new go.Binding("visible", "slotIp", function(s) { if (s) return false; else return true; }),
                        new go.Binding('source', '', function(data) { return '/img/rack/noInfo.png'})),
                    ...
                    $(go.TextBlock,
                        { row: 0, column: 0, width: 100, height: 20, verticalAlignment: go.Spot.Center, alignment: go.Spot.Left, font: 'bold 11px sans-serif', stroke: '#000', background: "#e5f1fa" },
                        new go.Binding('text', 'slotIp', function(v) { return Rack.infoRenderer(v, false); }),
                        new go.Binding("visible", "slotIp", function(s) { if (s) return true; else return false; })
                    ),

                    /* 임시.. 정보없음*/
                    $(go.Picture,{ row: 0, column: 0,  width: 40, height: 20,  background: "#e5f1fa", imageStretch: go.GraphObject.Uniform },
                        new go.Binding("visible", "slotKind", function(s) { if (s) return false; else return true; }),
                        new go.Binding('source', '', function(data) { return '/img/rack/noInfo.png'})),

                    $(go.TextBlock,
                        { row: 0, column: 0, width: 40, height: 20, verticalAlignment: go.Spot.Center, alignment: go.Spot.Left, font: 'bold 11px sans-serif', stroke: '#000', background: "#e5f1fa" },
                        new go.Binding('text', 'slotKind', function(v) { return Rack.infoRenderer(v, false); }),
                        new go.Binding("visible", "slotKind", function(s) { if (s) return true; else return false; })
                    ),
                    ...
                    /* 임시.. 정보없음*/
                    $(go.Picture,{ row: 0, column: 0,  width: 60, height: 20,  background: "#e5f1fa", imageStretch: go.GraphObject.Uniform },
                        new go.Binding("visible", "devKind2", function(s) { if (s) return false; else return true; }),
                        new go.Binding('source', '', function(data) { return '/img/rack/noInfo.png'})),
                    ...
                    // $(go.Panel, "Viewbox", {desiredSize: new go.Size(20,20)},
                    $(go.Panel, "Viewbox", {desiredSize: new go.Size(20,20), background: "#e5f1fa"},
                        $(go.Picture,{ desiredSize: new go.Size(20,20), width:20, height:20, imageStretch: go.GraphObject.Default },
                        new go.Binding("visible", "devKind2", function(s) { if (s) return true; else return false; }),
                        new go.Binding('source', '', function(data) { return Rack.infoDevNameRenderer(data.devKind1, data.devKind2); }))
                    ),
                    $(go.TextBlock,
                        { row: 0, column: 0, width: 120, height: 20, isUnderline : "underline", verticalAlignment: go.Spot.Center, alignment: go.Spot.Left, font: 'bold 11px sans-serif', stroke: '#000', background: "#e5f1fa" },
                        new go.Binding('text', 'devKind2', function(v) { return Rack.infoRenderer(v,true, 10); }),
                        new go.Binding("visible", "devKind2", function(s) {console.log(s); if (s) return true; else return false; }),
                        { click: function(e, obj) { Rack.showDetail(e, obj) } }
                    )
            ...
            $(go.Group,
                {
                    ...
                    resizable: false, movable: false, selectable: false, deletable: false,
                    locationSpot: go.Spot.TopLeft,
                    width: 390, height: Rack.rackSlotU * 20,
                    mouseDrop: function(e, grp) {
                        Rack.rackInfoDiagram.clearSelection();
                    }
                },
                new go.Binding('position', 'pos', go.Point.parse).makeTwoWay(go.Point.stringify)
            );
        ...
        this.rackInfoDiagram.nodeTemplateMap = nodeTemplateMap;
        ...
        this.rackInfoDiagram.groupTemplateMap = groupTemplateMap;

        // decide what kinds of Parts can be added to a Group
        this.rackInfoDiagram.commandHandler.memberValidation = function(grp, node) {
    ...
    **/
    setRackInfo: function (e, node) {
        ...
        if (_data.slotKind == Rack.SLOT_KIND.BLD || _data.slotKind == Rack.SLOT_KIND.BSVR) {
            if(_data.slotKind == Rack.SLOT_KIND.BLD){
                _infoNode = Rack.rackInfoDiagram.findNodeForKey(_data.group);
            }else if(_data.slotKind == Rack.SLOT_KIND.BSVR){
                _infoNode = Rack.rackInfoDiagram.findNodeForKey(_data.key);
            }
            $.each(Rack.rackInfoDiagram.model.nodeDataArray, function(idx, item) {
                if(_data.group == item.key){
                    Rack.rackInfoDiagram.model.setDataProperty(_infoNode.data, "evtKey", _data.key)
                    Rack.rackInfoDiagram.model.setDataProperty(_infoNode.data, "devKind1", _data.devKind1)
                    Rack.rackInfoDiagram.model.setDataProperty(_infoNode.data, "devKind2", _data.devKind2)
                    Rack.rackInfoDiagram.model.setDataProperty(_infoNode.data, "mngNo", _data.mngNo)
                    Rack.rackInfoDiagram.model.setDataProperty(_infoNode.data, "slotIp", _data.slotIp)
                    Rack.rackInfoDiagram.model.setDataProperty(_infoNode.data, "slotKind", _data.slotKind)
                    Rack.rackInfoDiagram.model.setDataProperty(_infoNode.data, "slotName", _data.slotName)
                }
            });
        }
    ...
    initEvent: function () {
        HmGrid.create($('#rackEvtGrid'), {
            source: new $.jqx.dataAdapter(
                {
                    datatype: 'json',
                    url: '/main/env/rackConf/getRackEvtList.do',
                },
                {
            ...
            rendertoolbar: function(toolbar) {
                HmGrid.titlerenderer(toolbar, 'Rack 이벤트');
            },

            columns:
                [
                    { text: '장애등급', datafield: 'evtLevel', width: 80,  cellsalign: 'center', cellsrenderer: HmGrid.evtLevelRenderer },
                    { text: '발생일시', datafield: 'ymdhms', width: 120, cellsalign: 'center' },
                    { text: '장애대상', datafield: 'srcInfo', minwidth: 180 },
                    { text: '이벤트명', datafield: 'evtName', width: 140 },
                    { text: '지속시간', datafield: 'sumSec', width: 130, cellsrenderer: HmGrid.cTimerenderer },
                ],
        });
    },
    ...
    initVirtualSvr: function () {
        ...
        HmGrid.create($('#p_virtualSvrGrid'), {
            ...
            columns: [
                {text: '그룹', datafield: 'grpName', width: '25%'},
                {text: '장비명', datafield: 'disDevName', width: '35%'},
                {text: '종류', datafield: 'devKind2', width: '20%'},
                {text: 'IP', datafield: 'devIp', width: '20%'},
                // {
                //     text: '등록구분', datafield: 'kind', width: 70,
