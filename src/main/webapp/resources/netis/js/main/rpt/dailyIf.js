...
var Main = {
    ...
    eventControl : function(event) {
        ...
        switch (curTarget.id) {
            ...
            // case 'btnCList_bps': this.showChartData(); break;
            // case 'btnCSave_bps': this.saveChart(); break;
    ...
    initDesign : function() {
        ...
        HmJqxSplitter.create($('#subSplitter'), HmJqxSplitter.ORIENTATION_H, [{ size: '60%', collapsible: false }, { size: '40%' }], 'auto', '100%');
...
//         Main.createDefaultHighChart('bpsChart', defaultSeriesArray, '');
		...
		dailyIfChart = new CustomChart('bpsChart', HmHighchart.TYPE_LINE, {series: dailyIfSeries, chartConfig: { unit: '1000' }
		}, '/main/rpt/dailyIf/getDailyIfChart.do');
		dailyIfChart.initialize();
    ...
    /** 일간회선 그리드 조회 */
    search : function() {
        HmGrid.updateBoundData($rptGrid, ctxPath + '/main/rpt/dailyIf/getDailyIfList.do');
    },
	...
	searchChart: function() {
		...
		dailyIfChart.searchData(params);
	},
   /* /!**  일간회선 차트 조회 *!/
    searchChart: function() {
        var rowIdx = HmGrid.getRowIdx($rptGrid, '회선을 선택해주세요.');
