...
var Main = {
	...
	/** init design */
	initDesign : function() {
		$('#mainSplitter').jqxSplitter({ width: '99.8%', height: '99.8%', orientation: 'vertical', theme: jqxTheme, panels: [{ size: 300 }, { size: '100%' }] });
		$('#splitter').jqxSplitter({ width: '100%', height: '100%', orientation: 'horizontal', theme: jqxTheme, panels: [{ size: '60%' }, { size: '40%', collasible: true }] });
...
//			}
//		});
		Master.createPeriodCondition($period, $('#date1'), $('#date2'));
		...
		$('#chartType').jqxDropDownList({ width: 80, height: 21, autoDropDownHeight: true, selectedIndex: 0,
			source: [{label: 'Byte', value: 'bytes'}, {label: 'Packet', value: 'pkt'}, {label: 'BPS', value: 'bps'}, {label: 'PPS', value: 'pps'}, {label: 'Host수', value: 'hostCnt'}]
		}).on('change', function(event) {
			Main.chgChartType();
		});
		var settings = HmChart2.getCommOptions(HmChart2.T_LINE, HmChart2.XUNIT_HOUR, HmChart2.unit1024FormatFn);
		$.extend(settings, {
			seriesGroups: [
			               	HmChart2.getSeriesGroup($progressChart, HmChart2.T_LINE, HmChart2.unit1024ToolTipFormatFn, 
			               			HmChart2.getSeries(['bytes'], ['Byte'])
		               		)
	               		]
		});
		HmChart2.create($progressChart, settings);
	},
	...
	createGrid: function($grid) {
		...
		$grid.on('rowselect', function(event) {
			Main.searchDetail(event.args.row);
		});
	},
	...
	chgGrpTab: function(tab) {
		var showTab = null;
		switch(tab) {
		case 0: showTab = [ TAB.ALL_USER, TAB.IN_USER, TAB.OUT_USER, TAB.MATRIX, TAB.MATRIX_PORT, TAB.APP, TAB.BIZ, TAB.GRP, TAB.AS ]; break;
	...
	chgChartType: function() {
		var _item = $('#chartType').jqxDropDownList('getSelectedItem');
		var _formatFn = null, _tooltipFormatFn = null, _seriesGroups = [];
		switch(_item.value) {
		case 'bytes':
			_formatFn = HmChart2.unit1024FormatFn;
			_tooltipFormatFn = HmChart2.unit1024ToolTipFormatFn;
			break;
		case 'pkt': case 'bps': case 'pps':
			_formatFn = HmChart2.unit1000FormatFn;
			_tooltipFormatFn = HmChart2.unit1000ToolTipFormatFn;
			break;
		case 'hostCnt':
			_formatFn = null;
			_tooltipFormatFn = null;
			break;
		default: return;
		}
		
		_seriesGroups = [
		                 	HmChart2.getSeriesGroup($progressChart, HmChart2.T_LINE, _tooltipFormatFn, 
		                 			HmChart2.getSeries([_item.value], [_item.label])
		                 	)
		                 ];
		var _tmp = $progressChart.jqxChart('valueAxis');
		_tmp.formatFunction = _formatFn;
		$progressChart.jqxChart('valueAxis', _tmp);
		$progressChart.jqxChart('seriesGroups', _seriesGroups);
	},
	
	/** 상세 추이 조회 */
	searchDetail: function(rowdata) {
		var params = Main.getCommParams();
		var url = null;
		switch($('#mainTabs').val()) {
		case TAB.MANG: 
			params.selGrpNo = rowdata.grpNo;
			url = '/getMangTrafficStatusChartList.do';
			break;
		case TAB.SUB_MANG:
			break;
		case TAB.ALL_USER: case TAB.IN_USER: case TAB.OUT_USER:
			params.ip = rowdata.hostIp;
			url = '/getIpTrafficStatusChartList.do';
			break;
		case TAB.IP: 
			break;
		case TAB.SVC: 
			break;
		case TAB.APP: 
			params.serviceNo = rowdata.grpNo;
			url = '/getGrpTrafficStatusChartList.do';
			break;
		case TAB.BIZ: 
			params.selGrpNo = rowdata.grpNo;
			url = '/getBizTrafficStatusChartList.do';
			break;
		case TAB.ISP: 
			break;
		case TAB.AS: 
			params.srcAs = rowdata.srcAs;
			params.dstAs = rowdata.dstAs;
			url = '/getAsTrafficStatusChartList.do';
			break;
		case TAB.SVR: 
			break;
		case TAB.MATRIX: 
			params.ip1 = rowdata.srcIp;
			params.ip2 = rowdata.dstIp;
			url = '/getMatrixTrafficStatusChartList.do';
			break;
		case TAB.MATRIX_PORT: 
			params.ip1 = rowdata.srcIp;
			params.ip2 = rowdata.dstIp;
			params.ip1Port = rowdata.srcPort;
			params.ip2Port = rowdata.dstPort;
			url = '/getMatrixPortTrafficStatusChartList.do';
			break;
		case TAB.GRP: 
			params.selGrpNo = rowdata.grpNo;
			url = '/getGrpTrafficStatusChartList.do';
			break;
		case TAB.SUB_GRP: 
			params.subNo = rowdata.grpNo;
			url = '/getSubGrpTrafficStatusChartList.do';
			break;
		}
		
		Main.drawChart(params, '/main/tms/trafficStatus/' + url);
	},
	
	/** 차트 draw */
	drawChart: function(params, url) {
		Server.get(url, {
			data: params,
			success: function(result) {
				$progressChart.jqxChart('source', result);
				$progressChart.jqxChart('update');
			}
		});
	},
