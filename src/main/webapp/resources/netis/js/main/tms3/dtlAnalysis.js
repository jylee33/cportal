...
var Main = {
	...
	getCommParams: function() {
		...
		if($.format.date($("#sDate1").val("date"), "yyyyMMdd")==$.format.date($("#sDate2").val("date"), "yyyyMMdd"))
		{
		    //날짜는 같은데 시간은 다른 경우
            if($.format.date($("#sDate1").val("date"), "HHmm")==$.format.date($("#sDate2").val("date"), "HHmm")){
                sameFlag = true;
            }else{
                sameFlag = false;
            }
        ...
        if(!sameFlag){
            //같은 시간이 아닐 때
            // 5분 데이터는 00분00초 ~ 04분59초 를 00분 5분 데이터로 보기 때문에 아래와 같이 처리
            var date2Str = HmDate.getDateStr($('#sDate2')) + '' + HmDate.getTimeStr($('#sDate2')) + '00';
            date2Str = date2Str.replace(/(\d{4})(\d{2})(\d{2})(\d{2})(\d{2})(\d{2})/, '$1-$2-$3 $4:$5:$6');
            var tempDate = new Date(date2Str);
            tempDate.setMinutes( tempDate.getMinutes() -1 );
            params.date2 = $.format.date(tempDate, "yyyyMMdd");
            params.time2 = $.format.date(tempDate, "HHmm");
        }
	...
	searchData: function() {
        ...
        if(timeGap > 3601 ) {
            alert('시간 간격은 1시간 간격만 가능합니다.');
            return false;
        }

		HmGrid.updateBoundData($dataGrid, ctxPath + '/main/tms3/dtlAnalysis/getDtlAnalysisList.do');
	},

	exportExcel: function() {
        HmUtil.exportGrid($dataGrid, '상세분석', false);
		// HmUtil.exportExcel(ctxPath + '/main/tms2/dtlAnalysis/export.do', Main.getCommParams());
	}
