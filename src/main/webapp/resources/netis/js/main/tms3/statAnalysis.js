...
var protoList , portList , serviceList, asList , appList , ispList , workList;
...
var Main = {
	...
	initVariable : function() {
        ...
        HmBoxCondition.createTmsDatePeriod();
	...
	initData : function() {
        ...
        Server.post('/main/tms3/trendAnalysis/getStatAppList.do', {
            data: {} ,
            success: function (result) {
                appList = result;
            }
        });
        Server.post('/main/tms3/trendAnalysis/getNtCfgIspList.do', {
            data: {} ,
            success: function (result) {
                console.dir(result);
                ispList = result;
	...
	createGrid: function() {
		...
		var _fixedCols = [
            { text: 'Bytes', datafield: 'bytes', width: 100, cellsrenderer: HmGrid.unit1024rendererUseTms, align: 'center', filtertype: 'number' },
            { text: 'Packets', datafield: 'pkts', width: 100, cellsrenderer: HmGrid.unit1000rendererUseTms, align: 'center', filtertype: 'number' },
            { text: 'BPS', datafield: 'bps', width: 100, cellsalign: 'right', cellsrenderer: HmGrid.unit1000rendererUseTms, align: 'center', filtertype: 'number' },
            { text: 'PPS', datafield: 'pps', width: 100, cellsalign: 'right', cellsrenderer: HmGrid.unit1000rendererUseTms, align: 'center', filtertype: 'number' },
            { text: 'Packet Size', datafield: 'pktsSize', width: 100, cellsalign: 'right', cellsformat: 'n', align: 'center', filtertype: 'number' , hidden : true }
        ];
		...
		switch(params.trfType) {
			...
			// 	break;
			case 'PROTOCOL':
				_dynamicCols.push({ text: '프로토콜', datafield: 'trfValue1', width: 120,  filtertype: 'checkedlist', align: 'center' });
				break;
			case 'APP':
				_dynamicCols.push({ text: 'APP', datafield: 'trfValue1', width: 250, align: 'left' ,
                    // cellsrenderer: function (row, column, value, rowData) {
                    //     var rData = $dataGrid.jqxGrid('getrowdata', row);
                    //     // var appStr = "protocol : "+value+" , port : "+rData.trfValue2+"";
                    //     var appStr = "-";
                    //     if (value != "") {
                    //         $.each(appList, function (idx, item) {
                    //             if (item.protocol == value && item.port == rData.trfValue2 ) {
                    //                 appStr = item.name +"("+ appStr +")";
                    //             }
                    //         });
                    //     }
                    //     return '<div style="margin-top: 6px" class="jqx-grid-cell-middle-align">' + appStr + '</div>';
                    // }
                });
				break;
            case 'AS':
                    ...
                    cellsrenderer: function (row, column, value, rowData) {
                        ...
                        if (value != "") {
                            $.each(asList, function (idx, item) {
                                if (item.asNo == value) {
                                    asStr = value + " (" + item.nameShort + ")";
                                }
                            });
                        }
                        return '<div style="margin-top: 6px" class="jqx-grid-cell-middle-align">' + asStr + '</div>';
                    }
                });
                break;
			case 'ISP':
				_dynamicCols.push({ text: 'ISP', datafield: 'trfValue1', width: 130, align: 'center' ,
                    cellsrenderer: function (row, column, value, rowData) {
                        var ispStr = value;
                        if (value != "") {
                            $.each(ispList, function (idx, item) {
                                if (item.ispNo == value) {
                                    ispStr = value + " (" + item.ispName + ")";
                                }
                            });
                        }
                        return '<div style="margin-top: 6px" class="jqx-grid-cell-middle-align">' + ispStr + '</div>';
                    }
                });
				break;
			case 'WORK':
                    ...
                    cellsrenderer: function (row, column, value, rowData) {
                        ...
                        if (value != "") {
                            $.each(workList, function (idx, item) {
                                if (item.serviceNo == value) {
                                    workStr = value + " (" + item.grpName + ")";
                                }
                            });
                        }
                        return '<div style="margin-top: 6px" class="jqx-grid-cell-middle-align">' + workStr + '</div>';
                    }
                });
				break;
            case 'GROUP':
	...
	searchData: function() {
        ...
        var toMonth = $('#sDate2').val('date').getMonth() + 1;
        var fromMonth = $('#sDate1').val('date').getMonth() + 1;
        var toDate = $('#sDate2').val('date').getDate();
        var fromDate = $('#sDate1').val('date').getDate();
        if( toMonth - fromMonth == 3 ){
            if( fromDate < toDate ){
                alert('기간은 3개월 간격만 가능합니다.');
                return false;
            }
		...
		// service call
		HmGrid.updateBoundData($dataGrid, ctxPath + '/main/tms3/statAnalysis/getStatAnalysisList.do');
	},
